...
export default function StudioGallery() {
  ...
  async function add() {
    if (!form.image_url) return toast.error('Image URL is required')
    setSaving(true)
    try {
      const { error } = await supabase.from('gallery').insert({ ...form, display_order: images.length })
      if (error) throw error
      toast.success('Photo added'); setModal(false); setForm({ image_url: '', title: '', category: 'cut', stylist_id: '' }); load()
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
      ...
      <div style={{ flexShrink: 0, marginBottom: '1.1rem', paddingBottom: '1.1rem', borderBottom: `1px solid ${C.border}`, display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between' }}>
        ...
        <button onClick={() => setModal(true)} className="btn-g"
          ...
          <div style={{ width: '100%', maxWidth: 460, background: C.modal, border: `1px solid ${C.goldBorder}`, borderRadius: 16, padding: '1.75rem', maxHeight: '90vh', overflowY: 'auto', boxShadow: '0 32px 80px rgba(0,0,0,.6)' }}
            ...
            <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: '1.5rem' }}>
              <div>
                ...
                <p style={{ fontSize: '0.75rem', color: C.muted, fontFamily: 'Jost,sans-serif' }}>Add to the gallery</p>
              </div>
              <button onClick={() => setModal(false)} style={{ width: 30, height: 30, borderRadius: '50%', background: C.subtle, border: `1px solid ${C.border}`, color: C.muted, display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer' }}>
                <X size={14} />
              </button>
            </div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
              <div>
                ...
                {form.image_url && <img src={form.image_url} alt="preview" style={{ marginTop: 8, height: 120, width: '100%', objectFit: 'cover', borderRadius: 8, border: `1px solid ${C.border}` }} />}
              </div>
            ...
            <div style={{ display: 'flex', gap: '0.625rem', marginTop: '1.5rem' }}>
              <button onClick={() => setModal(false)} style={{ flex: 1, padding: '0.6rem', borderRadius: 9, background: 'transparent', border: `1px solid ${C.border}`, color: C.muted, fontSize: '0.8rem', fontFamily: 'Jost,sans-serif', cursor: 'pointer' }}>Cancel</button>
