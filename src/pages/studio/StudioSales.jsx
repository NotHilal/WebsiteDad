        ...
        {summaryCards.map(({ label, value, sub, color, icon: Icon }) => (
          <div key={label} style={{ ...card, padding: '0.875rem 1rem', display: 'flex', alignItems: 'center', gap: 12 }}>
            ...
            <div style={{ minWidth: 0 }}>
              ...
              <p style={{ fontSize: '0.65rem', color: 'rgba(255,255,255,0.16)', fontFamily: 'DM Sans,sans-serif', marginTop: 1 }}>{loading ? '…' : sub}</p>
            </div>
          </div>
        ))}
        ...
        <div className="sales-tab-bar" style={{ flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'space-between', borderBottom: `1px solid ${C.border}`, padding: '0 1.25rem', flexWrap: 'wrap', gap: '0.25rem' }}>
          ...
          <div style={{ position: 'relative' }}>
            ...
            <input value={search} onChange={e => { setSearch(e.target.value); setMobilePage(0) }} autoComplete="off"
              ...
              style={{ background: 'rgba(255,255,255,0.04)', border: `1px solid ${C.border}`, borderRadius: 8, padding: '0.4rem 0.75rem 0.4rem 2rem', fontSize: '0.78rem', color: C.white, outline: 'none', fontFamily: 'DM Sans,sans-serif', width: 200, maxWidth: '100%', transition: 'border-color .2s' }}
              className="s-search" />
          </div>
        </div>
                ...
                <thead>
                  <tr style={{ background: 'rgba(255,255,255,0.02)', borderBottom: `1px solid ${C.border}`, position: 'sticky', top: 0 }}>
                    {['Booked', 'Appt. Date', 'Client', 'Service', 'Stylist', 'Status', 'Payment', 'Price'].map(h => (
                      <th key={h} style={{ padding: '0.55rem 1.1rem', fontSize: 9, letterSpacing: '0.16em', textTransform: 'uppercase', color: C.muted, fontWeight: 600, textAlign: 'left', fontFamily: 'DM Sans,sans-serif', background: '#161620' }}>{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                      ...
                      <tr key={appt.id} style={{ borderBottom: i < filteredAppts.length - 1 ? `1px solid ${C.border}` : 'none' }} className="s-row">
                        ...
                        <td style={{ padding: '0.6rem 1.1rem' }}>
                          {appt.payment_status === 'paid'
                            ...
                            : <span style={{ fontSize: 10, padding: '3px 10px', borderRadius: 20, background: 'rgba(255,255,255,0.05)', border: `1px solid ${C.border}`, color: C.muted, fontFamily: 'DM Sans,sans-serif', fontWeight: 600 }}>Unpaid</span>
                          }
                        </td>
                    ...
                    <div key={appt.id} style={{ padding: '0.875rem 1rem', minHeight: 70, borderBottom: i < MOBILE_PER_PAGE - 1 ? `1px solid ${C.border}` : 'none', borderLeft: `3px solid ${s.color}` }}>
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', gap: 7, marginBottom: 6 }}>
                        ...
                        {paid
                          ...
                          : <span style={{ fontSize: 10, padding: '3px 8px', borderRadius: 20, background: 'rgba(255,255,255,0.05)', border: `1px solid ${C.border}`, color: C.muted, fontFamily: 'DM Sans,sans-serif', fontWeight: 600, flexShrink: 0 }}>Unpaid</span>
                        }
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', gap: 5, flexWrap: 'wrap' }}>
                        ...
                        {appt.services?.name && <><span style={{ color: 'rgba(255,255,255,0.1)' }}>·</span><span style={{ fontSize: '0.7rem', color: 'rgba(255,255,255,0.35)', fontFamily: 'DM Sans,sans-serif', overflow: 'hidden', whiteSpace: 'nowrap', textOverflow: 'ellipsis', maxWidth: 140 }}>{appt.services.name}</span></>}
                        {appt.stylists?.name && <><span style={{ color: 'rgba(255,255,255,0.1)' }}>·</span><span style={{ fontSize: '0.7rem', color: 'rgba(255,255,255,0.28)', fontFamily: 'DM Sans,sans-serif' }}>{appt.stylists.name}</span></>}
                        {appt.created_at && <><span style={{ color: 'rgba(255,255,255,0.1)' }}>·</span><span style={{ fontSize: '0.68rem', color: 'rgba(255,255,255,0.2)', fontFamily: 'DM Sans,sans-serif' }}>booked {format(new Date(appt.created_at), 'MMM d')}</span></>}
                      </div>
                    </div>
                  )
                })}
                  ...
                  return (
                    <div key={order.id} style={{ padding: '0.875rem 1rem', minHeight: 70, borderBottom: i < MOBILE_PER_PAGE - 1 ? `1px solid ${C.border}` : 'none', borderLeft: `3px solid ${s.color}` }}>
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', gap: 5, flexWrap: 'wrap', paddingLeft: 33 }}>
                        {order.profiles?.full_name && <span style={{ fontSize: '0.7rem', color: 'rgba(255,255,255,0.38)', fontFamily: 'DM Sans,sans-serif' }}>{order.profiles.full_name}</span>}
                        <span style={{ color: 'rgba(255,255,255,0.1)' }}>·</span>
                        <span style={{ fontSize: '0.7rem', color: 'rgba(255,255,255,0.28)', fontFamily: 'DM Sans,sans-serif' }}>×{order.quantity || 1}</span>
                        {order.products?.price && <><span style={{ color: 'rgba(255,255,255,0.1)' }}>·</span><span style={{ fontSize: '0.7rem', color: 'rgba(255,255,255,0.28)', fontFamily: 'DM Sans,sans-serif' }}>${order.products.price} each</span></>}
                        <span style={{ color: 'rgba(255,255,255,0.1)' }}>·</span>
                        <span style={{ fontSize: '0.72rem', color: paid ? C.gold : 'rgba(255,255,255,0.32)', fontFamily: 'DM Sans,sans-serif', fontWeight: paid ? 700 : 400 }}>${lineTotal.toFixed(2)}</span>
                      </div>
                    </div>
                  )
                })}
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
      ...
      <div style={{ display: 'flex', gap: mode === 'month' ? 2 : 5 }}>
        {data.map(({ day, label, total }, i) => {
          const today = isToday(day)
          return (
            <div key={i} className="bar-label" style={{ flex: 1, textAlign: 'center', fontSize: mode === 'month' ? 8 : 10, color: today ? C.goldDim : total > 0 ? 'rgba(255,255,255,0.35)' : 'rgba(255,255,255,0.12)', fontFamily: 'DM Sans,sans-serif', transition: 'color .2s', fontWeight: today ? 700 : 400 }}>
              {label}
            </div>
          )
        })}
      ...
      <div style={{ display: 'flex', gap: 12, marginTop: 2 }}>
        {[['#34d399', 'Services'], ['#a78bfa', 'Products']].map(([col, lbl]) => (
          <div key={lbl} style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
            ...
            <span style={{ fontSize: 9, color: 'rgba(255,255,255,0.28)', fontFamily: 'DM Sans,sans-serif' }}>{lbl}</span>
          </div>
        ))}
      </div>
    </div>
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
      ...
      <div style={{ display: 'flex', gap: 5 }}>
        {counts.map(({ h, total }) => (
          <div key={h} className="bar-label" style={{ flex: 1, textAlign: 'center', fontSize: 10, color: total > 0 ? 'rgba(255,255,255,0.35)' : 'rgba(255,255,255,0.12)', fontFamily: 'DM Sans,sans-serif', transition: 'color .2s' }}>{h}h</div>
        ))}
      </div>
      <div style={{ display: 'flex', gap: 14, marginTop: 2 }}>
        {[['completed', C.gold], ['confirmed', '#34d399'], ['pending', '#f59e0b']].map(([s, col]) => (
          <div key={s} style={{ display: 'flex', alignItems: 'center', gap: 5 }}>
            <div style={{ width: 7, height: 7, borderRadius: 2, background: col }} />
            <span style={{ fontSize: 9, color: 'rgba(255,255,255,0.3)', fontFamily: 'DM Sans,sans-serif', textTransform: 'capitalize', letterSpacing: '0.1em' }}>{s}</span>
          </div>
        ))}
      </div>
    </div>
        ...
        {counts.map(({ s, n, col }) => (
          <div key={s} style={{ background: `${col}10`, border: `1px solid ${col}22`, borderRadius: 8, padding: compact ? '0.35rem 0.6rem' : '0.5rem 0.75rem', display: 'flex', alignItems: 'center', gap: 7 }}>
            ...
            <div>
              ...
              <p style={{ fontSize: 7, letterSpacing: '0.1em', textTransform: 'capitalize', color: 'rgba(255,255,255,0.25)', fontFamily: 'DM Sans,sans-serif', marginTop: 1 }}>{s}</p>
            </div>
          </div>
        ))}
      </div>
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 7 }}>
          ...
          <span style={{ fontSize: 10, color: 'rgba(255,255,255,0.35)', fontFamily: 'DM Sans,sans-serif' }}>Services <span style={{ color: '#34d399', fontWeight: 600 }}>${servicesRevenue.toFixed(2)}</span></span>
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 7 }}>
          <div style={{ width: 8, height: 8, borderRadius: 2, background: '#a78bfa' }} />
          <span style={{ fontSize: 10, color: 'rgba(255,255,255,0.35)', fontFamily: 'DM Sans,sans-serif' }}>Products <span style={{ color: '#a78bfa', fontWeight: 600 }}>${productsRevenue.toFixed(2)}</span></span>
        </div>
      </div>
    </div>
  )
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0.6rem 1rem', borderTop: `1px solid rgba(255,255,255,0.06)` }}>
      <button onClick={onPrev} disabled={page === 0}
        style={{ display: 'flex', alignItems: 'center', gap: 4, padding: '5px 10px', borderRadius: 8, background: 'none', border: `1px solid ${page === 0 ? 'rgba(255,255,255,0.06)' : 'rgba(255,255,255,0.12)'}`, color: page === 0 ? 'rgba(255,255,255,0.15)' : 'rgba(255,255,255,0.5)', fontSize: 11, fontFamily: 'DM Sans,sans-serif', cursor: page === 0 ? 'default' : 'pointer', transition: 'all .15s' }}>
      ...
      <button onClick={onNext} disabled={page >= total - 1}
        style={{ display: 'flex', alignItems: 'center', gap: 4, padding: '5px 10px', borderRadius: 8, background: 'none', border: `1px solid ${page >= total - 1 ? 'rgba(255,255,255,0.06)' : 'rgba(255,255,255,0.12)'}`, color: page >= total - 1 ? 'rgba(255,255,255,0.15)' : 'rgba(255,255,255,0.5)', fontSize: 11, fontFamily: 'DM Sans,sans-serif', cursor: page >= total - 1 ? 'default' : 'pointer', transition: 'all .15s' }}>
...
function Empty({ icon: Icon, text }) {
  return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: 160, gap: 8 }}>
      ...
      <p style={{ color: 'rgba(255,255,255,0.22)', fontSize: '0.82rem', fontFamily: 'DM Sans,sans-serif' }}>{text}</p>
    </div>
  )
}
