      ...
      <div style={{ flexShrink: 0 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '0.625rem', flexWrap: 'wrap', gap: '0.5rem' }}>
          <div>
            ...
            <p style={{ fontSize: '0.7rem', color: C.muted, fontFamily: 'DM Sans,sans-serif', marginTop: 3 }}>Revenue overview</p>
          </div>
        ...
        {/* Date navigation */}
        {mode === 'all'
          ? <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'rgba(var(--rgb-hi),0.025)', border: `1px solid ${C.border}`, borderRadius: 12, padding: '0.6rem 1rem' }}>
              <span className="font-display" style={{ fontSize: '0.95rem', color: C.gold, letterSpacing: '0.05em' }}>All time</span>
            </div>
          : <div style={{ display: 'flex', alignItems: 'center', background: 'rgba(var(--rgb-hi),0.025)', border: `1px solid ${C.border}`, borderRadius: 12, overflow: 'hidden' }}>
              ...
              <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 10, padding: '0.5rem 1rem' }}>
                ...
                {currentPeriod
                  ? <span style={{ fontSize: 9, letterSpacing: '0.18em', textTransform: 'uppercase', color: C.goldDim, fontFamily: 'DM Sans,sans-serif', fontWeight: 600, padding: '2px 9px', borderRadius: 20, background: C.goldBg, border: `1px solid ${C.goldBorder}`, flexShrink: 0 }}>{periodBadge}</span>
                  : <button onClick={() => setAnchor(new Date())} className="s-back-btn"
                      style={{ fontSize: 9, letterSpacing: '0.14em', textTransform: 'uppercase', color: C.goldDim, fontFamily: 'DM Sans,sans-serif', fontWeight: 700, padding: '3px 10px', borderRadius: 20, background: 'transparent', border: `1px solid ${C.goldBorder}`, cursor: 'pointer', transition: 'all .18s', whiteSpace: 'nowrap', flexShrink: 0 }}>
                      ↩ {backLabel}
                    </button>
                }
              </div>
        ...
        {summaryCards.map(({ label, value, sub, color, icon: Icon }) => (
          <div key={label} style={{ ...card, padding: '0.875rem 1rem', display: 'flex', alignItems: 'center', gap: 12 }}>
            ...
            <div style={{ minWidth: 0 }}>
              ...
              <p style={{ fontSize: 9, letterSpacing: '0.13em', textTransform: 'uppercase', color: C.muted, fontFamily: 'DM Sans,sans-serif', fontWeight: 600 }}>{label}</p>
              <p style={{ fontSize: '0.65rem', color: 'var(--col-text)', fontFamily: 'DM Sans,sans-serif', marginTop: 1 }}>{loading ? '…' : sub}</p>
            </div>
          </div>
        ))}
        <div style={{ ...card, padding: '0.875rem 1rem' }}>
          <p style={{ fontSize: 9, letterSpacing: '0.18em', textTransform: 'uppercase', color: C.muted, fontFamily: 'DM Sans,sans-serif', fontWeight: 600, marginBottom: '0.5rem' }}>Status</p>
          {loading
      ...
      <div className="sales-chart-card" style={{ ...card, flexShrink: 0, padding: '1rem 1.5rem' }}>
        <div className="sales-chart-row" style={{ display: 'flex', gap: '1.75rem', alignItems: 'flex-start' }}>
          <div style={{ flex: 1, minWidth: 0 }}>
            <p style={{ fontSize: 9, letterSpacing: '0.18em', textTransform: 'uppercase', color: C.muted, fontFamily: 'DM Sans,sans-serif', fontWeight: 600, marginBottom: '0.75rem' }}>
          ...
          {!loading && totalRevenue > 0 && (
            <div style={{ width: 200, flexShrink: 0, paddingLeft: '1.75rem', borderLeft: `1px solid ${C.border}` }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.75rem' }}>
                <p style={{ fontSize: 9, letterSpacing: '0.18em', textTransform: 'uppercase', color: C.muted, fontFamily: 'DM Sans,sans-serif', fontWeight: 600 }}>Revenue Split</p>
        ...
        <div className="sales-tab-bar" style={{ flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'space-between', borderBottom: `1px solid ${C.border}`, padding: '0 1.25rem', flexWrap: 'wrap', gap: '0.25rem' }}>
          <div style={{ display: 'flex' }}>
            {TABS.map(t => (
              <button key={t} onClick={() => { setTab(t); setSearch(''); setMobilePage(0) }} style={{
                ...
                fontSize: 11, textTransform: 'uppercase', letterSpacing: '0.14em', fontFamily: 'DM Sans,sans-serif',
                ...
                {t}
                <span style={{ marginLeft: 6, fontSize: 9, padding: '1px 7px', borderRadius: 20, background: tab === t ? C.goldBg : 'rgba(var(--rgb-hi),0.05)', color: tab === t ? C.goldDim : 'var(--col-text)' }}>
          ...
          <div style={{ position: 'relative' }}>
            ...
            <input value={search} onChange={e => { setSearch(e.target.value); setMobilePage(0) }} autoComplete="off"
              ...
              style={{ background: 'rgba(var(--rgb-hi),0.04)', border: `1px solid ${C.border}`, borderRadius: 8, padding: '0.4rem 0.75rem 0.4rem 2rem', fontSize: '0.78rem', color: C.white, outline: 'none', fontFamily: 'DM Sans,sans-serif', width: 200, maxWidth: '100%', transition: 'border-color .2s' }}
              className="s-search" />
          </div>
        </div>
                ...
                <thead>
                  <tr style={{ background: 'rgba(var(--rgb-hi),0.02)', borderBottom: `1px solid ${C.border}`, position: 'sticky', top: 0 }}>
                    {['Booked', 'Appt. Date', 'Client', 'Service', 'Stylist', 'Status', 'Payment', 'Price'].map(h => (
                      <th key={h} style={{ padding: '0.55rem 1.1rem', fontSize: 9, letterSpacing: '0.16em', textTransform: 'uppercase', color: C.muted, fontWeight: 600, textAlign: 'left', fontFamily: 'DM Sans,sans-serif', background: 'var(--col-modal)' }}>{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {filteredAppts.map((appt, i) => {
                    ...
                    return (
                      <tr key={appt.id} style={{ borderBottom: i < filteredAppts.length - 1 ? `1px solid ${C.border}` : 'none' }} className="s-row">
                        <td style={{ padding: '0.6rem 1.1rem', fontFamily: 'DM Sans,sans-serif', fontSize: '0.75rem', color: C.muted, whiteSpace: 'nowrap' }}>
                          {appt.created_at ? format(new Date(appt.created_at), mode === 'day' ? 'HH:mm' : 'MMM d HH:mm') : '—'}
                        </td>
                        <td style={{ padding: '0.6rem 1.1rem', fontFamily: 'DM Sans,sans-serif', fontSize: '0.75rem', color: C.gold, fontWeight: 600, whiteSpace: 'nowrap' }}>
                          {appt.date ? format(parseISO(appt.date), 'MMM d') : '—'} {appt.time?.slice(0, 5)}
                        </td>
                        <td style={{ padding: '0.6rem 1.1rem' }}>
                          <p style={{ color: C.white, fontSize: '0.78rem', fontFamily: 'DM Sans,sans-serif' }}>{appt.profiles?.full_name || '—'}</p>
                          {appt.profiles?.phone && <p style={{ color: C.muted, fontSize: '0.67rem', fontFamily: 'DM Sans,sans-serif' }}>{appt.profiles.phone}</p>}
                        </td>
                        <td style={{ padding: '0.6rem 1.1rem', color: C.dim, fontSize: '0.76rem', fontFamily: 'DM Sans,sans-serif' }}>
                          {appt.services?.name || '—'}
                          {appt.services?.duration && <span style={{ color: C.muted, fontSize: '0.67rem', marginLeft: 5 }}>{appt.services.duration}min</span>}
                        </td>
                        <td style={{ padding: '0.6rem 1.1rem', color: C.muted, fontSize: '0.76rem', fontFamily: 'DM Sans,sans-serif' }}>{appt.stylists?.name || '—'}</td>
                        <td style={{ padding: '0.6rem 1.1rem' }}>
                          <span style={{ fontSize: 10, padding: '3px 10px', borderRadius: 20, background: s.bg, border: `1px solid ${s.border}`, color: s.color, fontFamily: 'DM Sans,sans-serif', fontWeight: 600, textTransform: 'capitalize' }}>{appt.status}</span>
                        </td>
                        <td style={{ padding: '0.6rem 1.1rem' }}>
                          {appt.payment_status === 'paid'
                            ? <span style={{ fontSize: 10, padding: '3px 10px', borderRadius: 20, background: 'rgba(52,211,153,0.12)', border: '1px solid rgba(52,211,153,0.25)', color: '#34d399', fontFamily: 'DM Sans,sans-serif', fontWeight: 600 }}>Paid</span>
                            : <span style={{ fontSize: 10, padding: '3px 10px', borderRadius: 20, background: 'rgba(var(--rgb-hi),0.05)', border: `1px solid ${C.border}`, color: C.muted, fontFamily: 'DM Sans,sans-serif', fontWeight: 600 }}>Unpaid</span>
                          }
                        </td>
                        <td style={{ padding: '0.6rem 1.1rem', fontFamily: 'DM Sans,sans-serif', fontSize: '0.78rem', color: appt.payment_status === 'paid' ? C.gold : C.muted, fontWeight: appt.payment_status === 'paid' ? 600 : 400 }}>
                ...
                {paidAppts.length > 0 && !search && (
                  <tfoot>
                    <tr style={{ borderTop: `1px solid ${C.goldBorder}`, background: C.goldBg }}>
                      <td colSpan={7} style={{ padding: '0.55rem 1.1rem', fontSize: 9, letterSpacing: '0.16em', textTransform: 'uppercase', color: C.goldDim, fontFamily: 'DM Sans,sans-serif', fontWeight: 700 }}>
              ...
              <div className="s-mobile-list">
                  ...
                  return (
                    <div key={appt.id} style={{ padding: '0.875rem 1rem', minHeight: 70, borderBottom: i < MOBILE_PER_PAGE - 1 ? `1px solid ${C.border}` : 'none', borderLeft: `3px solid ${s.color}` }}>
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', gap: 7, marginBottom: 6 }}>
                        ...
                        <span style={{ fontSize: 10, padding: '3px 8px', borderRadius: 20, background: s.bg, border: `1px solid ${s.border}`, color: s.color, fontFamily: 'DM Sans,sans-serif', fontWeight: 600, textTransform: 'capitalize', flexShrink: 0 }}>{appt.status}</span>
                        {paid
                          ? <span style={{ fontSize: 10, padding: '3px 8px', borderRadius: 20, background: 'rgba(52,211,153,0.12)', border: '1px solid rgba(52,211,153,0.25)', color: '#34d399', fontFamily: 'DM Sans,sans-serif', fontWeight: 600, flexShrink: 0 }}>Paid</span>
                          : <span style={{ fontSize: 10, padding: '3px 8px', borderRadius: 20, background: 'rgba(var(--rgb-hi),0.05)', border: `1px solid ${C.border}`, color: C.muted, fontFamily: 'DM Sans,sans-serif', fontWeight: 600, flexShrink: 0 }}>Unpaid</span>
                        }
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', gap: 5, flexWrap: 'wrap' }}>
                        {appt.date && <span style={{ fontSize: '0.7rem', color: C.gold, fontFamily: 'DM Sans,sans-serif', fontWeight: 600 }}>{format(parseISO(appt.date), 'MMM d')}{appt.time ? ` ${appt.time.slice(0,5)}` : ''}</span>}
                        {appt.services?.name && <><span style={{ color: 'var(--col-text)' }}>·</span><span style={{ fontSize: '0.7rem', color: 'var(--col-text)', fontFamily: 'DM Sans,sans-serif', overflow: 'hidden', whiteSpace: 'nowrap', textOverflow: 'ellipsis', maxWidth: 140 }}>{appt.services.name}</span></>}
                ...
                {paidAppts.length > 0 && !search && (
                  <div style={{ padding: '0.6rem 1rem', background: C.goldBg, borderTop: `1px solid ${C.goldBorder}`, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <span style={{ fontSize: 9, letterSpacing: '0.16em', textTransform: 'uppercase', color: C.goldDim, fontFamily: 'DM Sans,sans-serif', fontWeight: 700 }}>Revenue — {paidAppts.length} paid</span>
            ...
            filteredOrders.length === 0 ? <Empty icon={Package} text={`No product orders for ${emptyText}`} /> : (<>
              ...
              <table className="s-table" style={{ width: '100%', borderCollapse: 'collapse' }}>
                      ...
                      <tr key={order.id} style={{ borderBottom: i < filteredOrders.length - 1 ? `1px solid ${C.border}` : 'none' }} className="s-row">
                        <td style={{ padding: '0.6rem 1.1rem' }}>
                          <div style={{ display: 'flex', alignItems: 'center', gap: 9 }}>
                            ...
                            <span style={{ color: C.white, fontSize: '0.78rem', fontFamily: 'DM Sans,sans-serif' }}>{order.products?.name || '—'}</span>
                          </div>
                        </td>
                        <td style={{ padding: '0.6rem 1.1rem' }}>
                          <p style={{ color: C.dim, fontSize: '0.76rem', fontFamily: 'DM Sans,sans-serif' }}>{order.profiles?.full_name || '—'}</p>
                          {order.profiles?.phone && <p style={{ color: C.muted, fontSize: '0.67rem', fontFamily: 'DM Sans,sans-serif' }}>{order.profiles.phone}</p>}
                        </td>
                        <td style={{ padding: '0.6rem 1.1rem', color: C.dim, fontSize: '0.78rem', fontFamily: 'DM Sans,sans-serif', fontWeight: 600 }}>×{order.quantity || 1}</td>
                        <td style={{ padding: '0.6rem 1.1rem' }}>
                          <span style={{ fontSize: 10, padding: '3px 10px', borderRadius: 20, background: s.bg, border: `1px solid ${s.border}`, color: s.color, fontFamily: 'DM Sans,sans-serif', fontWeight: 600, textTransform: 'capitalize' }}>{order.status === 'active' ? 'Awaiting Pickup' : order.status}</span>
                        </td>
                        <td style={{ padding: '0.6rem 1.1rem' }}>
                        ...
                        <td style={{ padding: '0.6rem 1.1rem', color: C.muted, fontSize: '0.76rem', fontFamily: 'DM Sans,sans-serif' }}>{order.products?.price ? `$${order.products.price}` : '—'}</td>
                        <td style={{ padding: '0.6rem 1.1rem', fontFamily: 'DM Sans,sans-serif', fontSize: '0.78rem', color: order.payment_status === 'paid' ? C.gold : C.muted, fontWeight: order.payment_status === 'paid' ? 600 : 400 }}>
                ...
                {paidOrders.length > 0 && !search && (
                  <tfoot>
                    <tr style={{ borderTop: `1px solid ${C.goldBorder}`, background: C.goldBg }}>
                      <td colSpan={6} style={{ padding: '0.55rem 1.1rem', fontSize: 9, letterSpacing: '0.16em', textTransform: 'uppercase', color: C.goldDim, fontFamily: 'DM Sans,sans-serif', fontWeight: 700 }}>Revenue — paid via Stripe ({paidOrders.length})</td>
              ...
              <div className="s-mobile-list">
                  ...
                  return (
                    <div key={order.id} style={{ padding: '0.875rem 1rem', minHeight: 70, borderBottom: i < MOBILE_PER_PAGE - 1 ? `1px solid ${C.border}` : 'none', borderLeft: `3px solid ${s.color}` }}>
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', gap: 7, marginBottom: 6 }}>
                        ...
                        <span style={{ fontSize: 10, padding: '3px 8px', borderRadius: 20, background: s.bg, border: `1px solid ${s.border}`, color: s.color, fontFamily: 'DM Sans,sans-serif', fontWeight: 600, textTransform: 'capitalize', flexShrink: 0 }}>
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', gap: 5, flexWrap: 'wrap', paddingLeft: 33 }}>
                        {order.profiles?.full_name && <span style={{ fontSize: '0.7rem', color: 'var(--col-text)', fontFamily: 'DM Sans,sans-serif' }}>{order.profiles.full_name}</span>}
                        <span style={{ color: 'var(--col-text)' }}>·</span>
                        <span style={{ fontSize: '0.7rem', color: 'var(--col-text)', fontFamily: 'DM Sans,sans-serif' }}>×{order.quantity || 1}</span>
                        {order.products?.price && <><span style={{ color: 'var(--col-text)' }}>·</span><span style={{ fontSize: '0.7rem', color: 'var(--col-text)', fontFamily: 'DM Sans,sans-serif' }}>${order.products.price} each</span></>}
                        <span style={{ color: 'var(--col-text)' }}>·</span>
                        <span style={{ fontSize: '0.72rem', color: paid ? C.gold : 'var(--col-text)', fontFamily: 'DM Sans,sans-serif', fontWeight: paid ? 700 : 400 }}>${lineTotal.toFixed(2)}</span>
                      </div>
                    </div>
                  )
                })}
                ...
                {paidOrders.length > 0 && !search && (
                  <div style={{ padding: '0.6rem 1rem', background: C.goldBg, borderTop: `1px solid ${C.goldBorder}`, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <span style={{ fontSize: 9, letterSpacing: '0.16em', textTransform: 'uppercase', color: C.goldDim, fontFamily: 'DM Sans,sans-serif', fontWeight: 700 }}>Revenue — {paidOrders.length} paid</span>
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                  ...
                  {total === 0
                    ...
                    : (
                      <motion.div className="bar-fill"
                        ...
                        {total > 0 && mode !== 'month' && (
                          <span style={{ position: 'absolute', top: -15, left: '50%', transform: 'translateX(-50%)', fontSize: 9, color: today ? C.gold : '#34d399', fontFamily: 'DM Sans,sans-serif', fontWeight: 700, whiteSpace: 'nowrap' }}>
                            ${total.toFixed(0)}
                          </span>
                        )}
                      </motion.div>
      ...
      <div style={{ display: 'flex', gap: mode === 'month' ? 2 : 5 }}>
        {data.map(({ day, label, total }, i) => {
          const today = isToday(day)
          return (
            <div key={i} className="bar-label" style={{ flex: 1, textAlign: 'center', fontSize: mode === 'month' ? 8 : 10, color: today ? C.goldDim : total > 0 ? 'var(--col-text)' : 'rgba(var(--rgb-hi),0.12)', fontFamily: 'DM Sans,sans-serif', transition: 'color .2s', fontWeight: today ? 700 : 400 }}>
              {label}
            </div>
          )
        })}
      ...
      <div style={{ display: 'flex', gap: 12, marginTop: 2 }}>
        {[['#34d399', 'Services'], ['#a78bfa', 'Products']].map(([col, lbl]) => (
          <div key={lbl} style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
            ...
            <span style={{ fontSize: 9, color: 'var(--col-text)', fontFamily: 'DM Sans,sans-serif' }}>{lbl}</span>
          </div>
        ))}
      </div>
    </div>
...
function AllTimeChart({ appointments, preorders }) {
  ...
  if (allKeys.length === 0) return (
    <div style={{ height: 120, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      <p style={{ color: C.muted, fontSize: '0.78rem', fontFamily: 'DM Sans,sans-serif' }}>No paid revenue yet</p>
    </div>
  )
      ...
      <svg viewBox={`0 0 ${W} ${H}`} style={{ width: '100%', display: 'block' }} preserveAspectRatio="none">
        ...
        {[0.25, 0.5, 0.75].map(f => (
          <line key={f} x1={PL} y1={PT + (1 - f) * (H - PT - PB)} x2={W - PR} y2={PT + (1 - f) * (H - PT - PB)}
            stroke="rgba(var(--rgb-hi),0.04)" strokeWidth="1" />
        ))}
              ...
              {hovered === i && (
                <g>
                  <rect x={Math.min(ptS.x - 46, W - 96)} y={ptS.y - 46} width={92} height={40} rx="5"
                    fill="var(--col-card)" stroke="rgba(var(--rgb-hi),0.1)" strokeWidth="1" />
                  <text x={Math.min(ptS.x - 46, W - 96) + 8} y={ptS.y - 30}
                    fill="#34d399" style={{ fontSize: 9, fontFamily: 'DM Sans,sans-serif', fontWeight: 700 }}>
                    S ${ptS.sRev.toFixed(0)}
                  </text>
                  <text x={Math.min(ptS.x - 46, W - 96) + 8} y={ptS.y - 15}
                    fill="#a78bfa" style={{ fontSize: 9, fontFamily: 'DM Sans,sans-serif', fontWeight: 700 }}>
      ...
      <div style={{ display: 'flex', marginTop: 6, paddingLeft: PL, paddingRight: PR }}>
        {months.map((m, i) => (
          <div key={m.k} style={{ flex: 1, textAlign: 'center', fontSize: 8,
            color: hovered === i ? 'var(--col-text)' : 'var(--col-text)',
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                  ...
                  {total === 0
                    ...
                    : (
                      <motion.div className="bar-fill"
                        ...
                        style={{ width: '100%', background: color ? `linear-gradient(to top, ${color}, ${color}66)` : 'rgba(var(--rgb-hi),0.12)', borderRadius: '4px 4px 0 0', position: 'relative', transition: 'filter .2s' }}>
                        {total > 0 && <span style={{ position: 'absolute', top: -16, left: '50%', transform: 'translateX(-50%)', fontSize: 10, color: color || C.muted, fontFamily: 'DM Sans,sans-serif', fontWeight: 700, whiteSpace: 'nowrap' }}>{total}</span>}
                      </motion.div>
                    )
                  }
                </div>
      ...
      <div style={{ display: 'flex', gap: 5 }}>
        {counts.map(({ h, total }) => (
          <div key={h} className="bar-label" style={{ flex: 1, textAlign: 'center', fontSize: 10, color: total > 0 ? 'var(--col-text)' : 'rgba(var(--rgb-hi),0.12)', fontFamily: 'DM Sans,sans-serif', transition: 'color .2s' }}>{h}h</div>
        ))}
      </div>
      <div style={{ display: 'flex', gap: 14, marginTop: 2 }}>
        {[['completed', C.gold], ['confirmed', '#34d399'], ['pending', '#f59e0b']].map(([s, col]) => (
          <div key={s} style={{ display: 'flex', alignItems: 'center', gap: 5 }}>
            <div style={{ width: 7, height: 7, borderRadius: 2, background: col }} />
            <span style={{ fontSize: 9, color: 'var(--col-text)', fontFamily: 'DM Sans,sans-serif', textTransform: 'capitalize', letterSpacing: '0.1em' }}>{s}</span>
          </div>
        ))}
      </div>
    </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: compact ? '0.3rem' : '0.5rem' }}>
        {counts.map(({ s, n, col }) => (
          <div key={s} style={{ background: `${col}10`, border: `1px solid ${col}22`, borderRadius: 8, padding: compact ? '0.35rem 0.6rem' : '0.5rem 0.75rem', display: 'flex', alignItems: 'center', gap: 7 }}>
            ...
            <div>
              <p style={{ fontFamily: '"Cormorant Garamond", serif', fontSize: compact ? '0.95rem' : '1.1rem', color: n > 0 ? col : 'var(--col-text)', lineHeight: 1 }}>{n}</p>
              <p style={{ fontSize: 7, letterSpacing: '0.1em', textTransform: 'capitalize', color: 'var(--col-text)', fontFamily: 'DM Sans,sans-serif', marginTop: 1 }}>{s}</p>
            </div>
          </div>
        ))}
      </div>
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 7 }}>
          ...
          <span style={{ fontSize: 10, color: 'var(--col-text)', fontFamily: 'DM Sans,sans-serif' }}>Services <span style={{ color: '#34d399', fontWeight: 600 }}>${servicesRevenue.toFixed(2)}</span></span>
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 7 }}>
          <div style={{ width: 8, height: 8, borderRadius: 2, background: '#a78bfa' }} />
          <span style={{ fontSize: 10, color: 'var(--col-text)', fontFamily: 'DM Sans,sans-serif' }}>Products <span style={{ color: '#a78bfa', fontWeight: 600 }}>${productsRevenue.toFixed(2)}</span></span>
        </div>
      </div>
    </div>
  )
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0.6rem 1rem', borderTop: `1px solid rgba(var(--rgb-hi),0.06)` }}>
      <button onClick={onPrev} disabled={page === 0}
        style={{ display: 'flex', alignItems: 'center', gap: 4, padding: '5px 10px', borderRadius: 8, background: 'none', border: `1px solid ${page === 0 ? 'rgba(var(--rgb-hi),0.06)' : 'rgba(var(--rgb-hi),0.12)'}`, color: page === 0 ? 'var(--col-text)' : 'var(--col-text)', fontSize: 11, fontFamily: 'DM Sans,sans-serif', cursor: page === 0 ? 'default' : 'pointer', transition: 'all .15s' }}>
      ...
      <button onClick={onNext} disabled={page >= total - 1}
        style={{ display: 'flex', alignItems: 'center', gap: 4, padding: '5px 10px', borderRadius: 8, background: 'none', border: `1px solid ${page >= total - 1 ? 'rgba(var(--rgb-hi),0.06)' : 'rgba(var(--rgb-hi),0.12)'}`, color: page >= total - 1 ? 'var(--col-text)' : 'var(--col-text)', fontSize: 11, fontFamily: 'DM Sans,sans-serif', cursor: page >= total - 1 ? 'default' : 'pointer', transition: 'all .15s' }}>
