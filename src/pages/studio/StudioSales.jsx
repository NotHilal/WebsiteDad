...
export default function StudioSales() {
  ...
  const backLabelShort = mode === 'day' ? 'Today' : mode === 'week' ? 'This week' : 'This month'
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
      <div style={{ display: 'flex', gap: 4 }}>
        {/* Y-axis scale */}
        <div style={{ width: Y_W, flexShrink: 0, display: 'flex', flexDirection: 'column', justifyContent: 'space-between', alignItems: 'flex-end', paddingRight: 6, paddingBottom: 0 }}>
          {[maxVal, maxVal * 0.75, maxVal * 0.5, maxVal * 0.25, 0].map((v, i) => (
            <span key={i} style={{ fontSize: 9, color: 'rgba(var(--rgb-hi),0.35)', fontFamily: 'DM Sans,sans-serif', lineHeight: 1 }}>
              {v === 0 ? '0' : `$${v >= 1000 ? (v / 1000).toFixed(1) + 'k' : v.toFixed(0)}`}
            </span>
          ))}
        </div>
        {/* Chart area */}
      ...
      <div style={{ display: 'flex', gap: mode === 'month' ? 2 : 5, paddingLeft: Y_W + 4 }}>
        {data.map(({ day, label, total }, i) => {
          const today = isToday(day)
          return (
            <div key={i} className="bar-label" style={{ flex: 1, textAlign: 'center', fontSize: mode === 'month' ? 10 : 12, color: today ? C.goldDim : total > 0 ? 'var(--col-text)' : 'rgba(var(--rgb-hi),0.12)', fontFamily: 'DM Sans,sans-serif', transition: 'color .2s', fontWeight: today ? 700 : 400 }}>
              {label}
            </div>
          )
        })}
...
function AllTimeChart({ appointments, preorders }) {
  ...
  if (allKeys.length === 0) return (
    <div style={{ height: 120, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      <p style={{ color: C.muted, fontSize: '0.95rem', fontFamily: 'DM Sans,sans-serif' }}>No paid revenue yet</p>
    </div>
  )
              ...
              {hovered === i && (
                <g>
                  ...
                  <text x={Math.min(ptS.x - 46, W - 96) + 8} y={ptS.y - 30}
                    fill="#34d399" style={{ fontSize: 13, fontFamily: 'DM Sans,sans-serif', fontWeight: 700 }}>
                    S ${ptS.sRev.toFixed(0)}
                  </text>
                  <text x={Math.min(ptS.x - 46, W - 96) + 8} y={ptS.y - 15}
                    fill="#a78bfa" style={{ fontSize: 13, fontFamily: 'DM Sans,sans-serif', fontWeight: 700 }}>
...
function HourlyChart({ appointments, hours }) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
      <div style={{ display: 'flex', gap: 4 }}>
        {/* Y-axis scale */}
        <div style={{ width: Y_W, flexShrink: 0, display: 'flex', flexDirection: 'column', justifyContent: 'space-between', alignItems: 'flex-end', paddingRight: 4 }}>
          {[maxCount, Math.round(maxCount * 0.75), Math.round(maxCount * 0.5), Math.round(maxCount * 0.25), 0].map((v, i) => (
            <span key={i} style={{ fontSize: 9, color: 'rgba(var(--rgb-hi),0.35)', fontFamily: 'DM Sans,sans-serif', lineHeight: 1 }}>{v}</span>
          ))}
        </div>
        {/* Chart area */}
        <div style={{ flex: 1, position: 'relative' }}>
      ...
      <div style={{ display: 'flex', gap: 5, paddingLeft: Y_W + 4 }}>
        {counts.map(({ h, total }) => (
          <div key={h} className="bar-label" style={{ flex: 1, textAlign: 'center', fontSize: 12, color: total > 0 ? 'var(--col-text)' : 'rgba(var(--rgb-hi),0.12)', fontFamily: 'DM Sans,sans-serif', transition: 'color .2s' }}>{h}h</div>
        ))}
      </div>
